...
def rnn_layers(x, seq_length, training, hidden_num=100, layer_num=3, class_n=5, reuse=False):
    cells_fw = list()
    cells_bw = list()
    for i in range(layer_num):
        # cell_fw = BNLSTMCell(hidden_num,training = training)#,training)
        # cell_bw = BNLSTMCell(hidden_num,training = training)#,training)
        cell_fw = LSTMCell(hidden_num, reuse=reuse)
        cell_bw = LSTMCell(hidden_num, reuse=reuse)
    ...
    with tf.variable_scope('rnn_fnn_layer'):
        ...
        #                          name='weights')
        weight_out = tf.get_variable(name="weights", shape=[2, hidden_num],
        ...
        #                            name='weights_class')
        weight_class = tf.get_variable(name="weights_class", shape=[hidden_num, class_n],
                                       initializer=tf.truncated_normal_initializer(stddev=np.sqrt(2.0/(2 * hidden_num))))
