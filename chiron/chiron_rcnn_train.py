...
def train(valid_reads_num=100):
    ...
    error, predict = prediction(logits, seq_length, y)
    ...
    # print("batch_x", len(batch_x), len(batch_x[0]))
    ...
    for i in range(FLAGS.max_steps):
        ...
        feed_dict = {x: batch_x, seq_length: seq_len / ratio, y_indexs: indxs, y_values: values, y_shape: shape,
                     training: True}
        loss_val, _, predict2 = sess.run([ctc_loss, opt, predict], feed_dict=feed_dict)
...
if __name__ == "__main__":
    class Flags():
        def __init__(self):
            self.home_dir = "/Users/andrewbailey/CLionProjects/nanopore-RNN/chiron/"
            self.data_dir = self.home_dir + 'data/raw/'
