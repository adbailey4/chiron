...
def read_raw_data_sets(data_dir, seq_length=200, k_mer=1, valid_reads_num=0, alphabet=5):
    ...
    for name in os.listdir(data_dir):
        if name.endswith(".signal"):
            ...
            event += tmp_event
            # print("f_label", f_label.base[:10])
            # print("tmp_event", tmp_event[100:110])
    ...
    # print("valid_event_length", valid_event_length)
    # print("valid_label", valid_label)
    # print("valid_label_length", valid_label_length)
    train = DataSet(event=train_event, event_length=train_event_length, label=train_label,
                    label_length=train_label_length)
    valid = DataSet(event=valid_event, event_length=valid_event_length, label=valid_label,
